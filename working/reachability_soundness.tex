...
\end{lem}
\begin{proof}
  Taking arbitrary $\trace_0 \in \mathcal{T}$, and an arbitrary event $\event = (\_, l_0, \_) \in \eventset$,
  ...
  There are 2 cases, where $l_e = l$ and $l_e \neq l$.
  \\
  In case of $l_0 \neq l$, we know $\event \not in \trace$, then this Lemma is vacuously true.
    \\
    In case of $l_0 = l$, by the abstract Execution Trace computation, we know 
  ...
  \caseL{$c = c_{s1};c_{s2}$}
 By the induction hypothesis on $c_{s1}$ and $c_{s2}$ separately, we have this case proved.
  ...
  $\config{{\ewhile [b]^{l} \edo c_w, \trace}}
    ...
    % \config{{
    % \eskip,
    % \trace_0 \tracecat [(b, l, \etrue)] \tracecat \trace_1}}
  $.
  \\
  ...
  \config{{
  \eskip,
  \trace_0 \tracecat [(b, l, \etrue)] \tracecat \trace_w}}
$
\\
...
\config{{
\eskip,
\trace_0 \tracecat [(b, l, \etrue)] \tracecat \trace_w \tracecat \trace_1}}
$ for some $\trace_1 \in \mathcal{T}$ and $\trace = [(b, l, \etrue)] \tracecat \trace_w \tracecat \trace_1$.
\\
Then we have 3 cases: (1) $\event \eventeq (b, l, \etrue)$, 
(2) $\event \in \trace_w$ or 
(3) $\event \in \trace_1$.
  \\
In case of (1). $\event \eventeq (b, l, \etrue)$, we have $\absflow(c) = \absflow'(c;\clabel{\eskip}^{l_e}) = \{(l, \top, \init(c_w))\} \cup \cdots $, we have $\absevent = (l, \top, \init(c_w))$ and this case is proved.
\\
In case of (2). $\event \in \trace_w$,
by induction hypothesis on 
$c_w$ with the execution $\config{{
  c_w,
  \trace_0 \tracecat [(b, l, \etrue)]}}
  \xrightarrow{*} 
  \config{{
  \eskip,
  \trace_0 \tracecat [(b, l, \etrue)] \tracecat \trace_w}}$ and trace $\trace_w$, we know there is an abstract event of the form 
  $\absevent = (l_0, \_, \_ ) \in \absflow(c_w)$ where $\absflow(c_w) = \absflow'(c_w;\clabel{\eskip}^{l_e})$.
  \\
  By definition of $\absflow'$, we have 
  $ \absflow'(c_w;\clabel{\eskip}^{l_e}) = \absflow'(c_w) \cup  \{ (l', dc, l_e) | (l', dc) \in \absfinal(c_w) \} $.
  \\
  There are 2 subcases: (2.1) $\absevent \in \absflow'(c_w)$ or $ (2.2) \absevent \in \{ (l', dc, l_e) | (l', dc) \in \absfinal(c_w) \}$
  \\
  \subcaseL{(2.1)}
  Since $\absflow(c) = \absflow'(c_w) \cup \{(l', dc, l)| (l', dc) \in \absfinal(c_w) \} \cup \cdots $, 
  we know the abstract event $\absevent$ of the form $\absevent = (l_0, \_, \_ ) \in \absflow(c)$. 
  \\
  This case is proved.
  \subcaseL{(2.2)}
  Since $\absflow(c) = \absflow'(c_w) \cup \{(l', dc, l)| (l', dc) \in \absfinal(c_w) \} \cup \cdots $, 
  we know there is an abstract event of the form $\absevent' = (l', \_, \_)$ for some $dc'$ and $(l', dc') \in \absfinal(c_w)$
   $\absevent = (l_0, \_, \_ ) \in \{(l', dc, l)| (l', dc) \in \absfinal(c_w) \}$, i.e., $(l_0, dc') \in \absfinal(c_w)$ for some $dc'$.
   Then the abstract event $\absevent$ of the form $\absevent = (l_0, \_, \_ ) \in \absflow(c)$.
  \\
  This case is proved.
  \\
  %
In case of (3). $\event \in \trace_1$, we know either $\event = (b, l, \_)$, or $\event \in \trace_w'$ where $\trace_w' \in \mathcal{T}$ is the trace of executing $c_w$ in an iteration.
\\
