...
\\ 
  ...
  \begin{array}{ll}   
    ...
    \bigwedge &  \tlabel_{\trace_3} \cap \tlabel_{\trace_3'} = \emptyset
     \land \vcounter(\trace') ~  \pi_2(\event_b) = \vcounter(\trace) ~ {\tt label}(\event_b)
...
The set of query variables of a program is the set of variables set to the result of a query in the program.
\mg{In the next definition, why do you call it a vector? It seems that you define it as a set.}\\
\jl{fixed}\\
\mg{Also, there is still confusion between labelled variables and pairs (variable, label).}\\
\jl{TODO: fix for event}
%
% \begin{defn}[Query Variables ($\qvar_{c} \subseteq \mathcal{VAR} \times \mathbb{N}$)].
  % \\
\begin{defn}[Query Variables ($\qvar: \cdom \to \mathcal{LVAR}$)] 
...
$\qvar$ is a set containing all variables newly assigned by the result of a query in the programm, $\qvar \subset \mathcal{VAR}$.
% \jl{fixed}
...
\jl{TODO}
%
\begin{lem}[Uniquness of the Labeled Variables]
\end{lem}
%
...
\mg{what is ``its'' referring to in the sentence below?}\\
...
\jl{I'm trying to define the length by just counting the number of query variables in the walk, instead of counting all 
the vertices.}
